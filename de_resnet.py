...
class BottleneckModified(nn.Module):
    ...
    def __init__(
        ...
        if stride == 2:
            # self.conv2 = deconv2x2(width, width, stride, groups, dilation)
            self.conv2_row = nn.ConvTranspose2d(in_channels=width, out_channels=width, stride=(1, stride), kernel_size=(1, 2), bias=False, groups=groups, dilation=dilation)
            self.conv2_col = nn.ConvTranspose2d(in_channels=width, out_channels=width, stride=(stride, 1), kernel_size=(2, 1), bias=False, groups=groups, dilation=dilation)
        else:
            # self.conv2 = conv3x3(width, width, stride, groups, dilation)
...
# ---------------------------------------------------------------------------- #
#        modified DeResNet: conv1 is replaced by Conv1d (change nothing)       #
# ---------------------------------------------------------------------------- #
'''
class ModifiedDeResNet(nn.Module):
    ...
    def __init__(
        ...
        super(ModifiedDeResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
# ---------------------------------------------------------------------------- #
#                modified DeResNet: layer1 is replaced by Conv1d               #
# ---------------------------------------------------------------------------- #
...
class ModifiedDeResNet(nn.Module):
    ...
    def __init__(
        ...
        #self.relu = nn.ReLU(inplace=True)
        #self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(BottleneckModified, 256, layers[0], stride=2)
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
...
def de_wide_resnet50_2_modified(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    kwargs['width_per_group'] = 64 * 2
    model = ModifiedDeResNet(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls['wide_resnet50_2'], progress=progress)
        # Load only layers that are unchanged (you may need to filter some keys depending on your changes)
        # Create a new state_dict that only includes matching keys
        modified_state_dict = {k: v for k, v in state_dict.items() if k in model.state_dict() and 'conv1_' not in k}
