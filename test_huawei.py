...
def evaluation(encoder, bn, decoder, dataloader,device,_class_=None):
    ...
    with torch.no_grad():
        ...
        for img, gt, label, _ in dataloader:
            ...
            img = img.to(device)
            inputs = encoder(img) # inputs是teacher网络（也即encoder）不同特征层输出特征图的列表
            outputs = decoder(bn(inputs)) # bn(inputs)是Bottleneck网络的输出，作为student网络（也即decoder）的输入; outputs是student网络不同特征层输出特征图的列表
        ...
        ## image-level: ROC curve, PR curve
        image_fpr, image_tpr, _ = roc_curve(gt_list_sp,pr_list_sp)
        # image-level: ROC curve
        fig_image_roc, axis = plot_roc_curve(image_fpr, image_tpr, auroc_sp)
        fig_image_roc.savefig('results/{}_image_roc_curve.png'.format(_class_))
        # image-level: PR curve
        precision,recall,_ = precision_recall_curve(gt_list_sp,pr_list_sp)
        fig_image_pr, axis = plot_pr_curve(recall,precision,ap_sp)
        fig_image_pr.savefig('results/{}_image_pr_curve.png'.format(_class_))

        # ## pixel-level: PRO curve, ROC curve

        # # pixel-level: PRO curve

        # # pixel-level: ROC curve
        pixel_fpr, pixel_tpr, _ = roc_curve(gt_list_px, pr_list_px)
        fig_pixel_roc, axis = plot_roc_curve(pixel_fpr, pixel_tpr, auroc_px)
        fig_pixel_roc.savefig('results/{}_pixel_roc_curve.png'.format(_class_))

    return auroc_px, auroc_sp, round(np.mean(aupro_list),3),ap_sp
...
def test(_class_):
    ...
    auroc_px, auroc_sp, aupro_px, ap_sp = evaluation(encoder, bn, decoder, test_dataloader, device,_class_)
    # print(_class_,':',auroc_px,',',auroc_sp,',',aupro_px,',',ap_sp)
    print(_class_,':')
    print('Sample Auroc{:.3f}, Sample AP{:.3f}, Pixel Auroc:{:.3f}, Pixel Aupro{:.3} '.format(auroc_sp, ap_sp, auroc_px, aupro_px))
    return auroc_px
...
def plot_roc_curve(fpr, tpr, auroc):
    ...
    axis.plot(fpr, tpr, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % auroc)
...
def plot_pr_curve(recall,precision,ap):
    fig, axis = plt.subplots()
    axis.plot(recall,precision,color='darkorange',lw=2,label='PR curve (area = %0.2f)' % ap)
    axis.set_xlim([0.0, 1.0])
    axis.set_ylim([0.0, 1.05])
...
# Example usage:
# fig, axis = plot_roc_curve(fpr, tpr, auroc_sp)
# plt.show()
